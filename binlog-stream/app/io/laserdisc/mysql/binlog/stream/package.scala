...
package object stream {
  def streamEvents[F[_]: ConcurrentEffect: SelfAwareStructuredLogger](
    ...
    _.through(streamTransactionPackages[F](transactionState)).flatMap(
      pkg =>
        fs2.Stream.eval(warnBigTransactionPackage(pkg)) >> fs2.Stream(
          pkg.events: _*))
  ...
  def streamTransactionPackages[
      F[_]: ConcurrentEffect: SelfAwareStructuredLogger](
      transactionState: Ref[F, TransactionState]
    ...
    _.evalMap(
      event =>
        SelfAwareStructuredLogger[F]
          ...
          .modifyState(TransactionState.nextState(event))).unNone
  ...
  def warnBigTransactionPackage[F[_]: Sync: SelfAwareStructuredLogger](
    ...
    if (transactionPackage.events.size >= 1000)
      for {
        ...
        _ <- SelfAwareStructuredLogger[F].warn(
          s"""Transaction has > then 1000 elements in it with
