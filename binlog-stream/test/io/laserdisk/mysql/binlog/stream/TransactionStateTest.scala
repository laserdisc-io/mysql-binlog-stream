...
import _root_.io.laserdisk.mysql.binlog._
...
class TransactionStateTest extends AnyWordSpec with Matchers with OptionValues {
  ...
  def createInsertEvent(): Event = {
    ...
    val rows: util.List[Array[io.Serializable]] = new util.ArrayList[Array[io.Serializable]]()
    rows.add(
    ...
    "accumulate events within transaction" in {
      ...
      res
        ...
        case (state, transactionPackage) =>
          ...
          transactionPackage.value.events.forall(_.fileName == "file.123") should be(true)
